...
public class MyoDissOpposedActinsEvaluator extends Evaluator {
	...
	public String getHeaderString() {
		...
		for(int i = 0; i < Actin.actinCt; i++) {
			s = s + "Actin" + i + "\t" + "numDetachmentEvents\tmean lifetime\tfinalAttachmentTime\tdurationOfLastAttachment\t";
		}
		return s;
	}
	...
	public String getDataString() {
		...
		lifetimes.add(new Double(Sim2D.simulationTime-firstAttchTime));
		...
		s = s + firstAttchTime + "\t";
		s = s + (Sim2D.simulationTime - firstAttchTime) + "\t";
		dataPW.close();
		return s;
	}
